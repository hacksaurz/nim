...
def test_new_game_success(client_app, request):
    ...
    response = client_app.post(
        "/new", content_type="application/json", data=json.dumps(request)
    )
...
def test_constants():
    ...
    assert all(
        ...
        for x in (DEFAULT_MIN_STONES_PER_PILE, DEFAULT_MAX_STONES_PER_PILE)
    )
